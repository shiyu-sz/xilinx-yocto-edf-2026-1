...
# This is done by creating a new /etc/sudoers.d/99-${DISTRO}.conf
...
set_sudoers () {
	...
	while test "x$setting" != "x"; do
		eval "$PSEUDO echo \"$setting\" >> \"${IMAGE_ROOTFS}\"/etc/sudoers.d/99-${DISTRO}"
		setting=`echo $remaining | cut -d ';' -f1`
		remaining=`echo $remaining | cut -d ';' -f2-`
	done
	if [ -f "${IMAGE_ROOTFS}/etc/sudoers.d/99-${DISTRO}" ]; then
		eval "$PSEUDO chmod 0440 \"${IMAGE_ROOTFS}\"/etc/sudoers.d/99-${DISTRO}"
	fi
}
