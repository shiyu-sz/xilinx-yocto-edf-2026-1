...
unsigned long ps7_clock_init_data_3_0[] = {
    ...
    // .. ..     ==> MASK : 0x03F00000U    VAL : 0x00500000U
    // .. .. 
    EMIT_MASKWRITE(0XF8000190, 0x03F03F30U ,0x00501A00U),
    // .. .. SRCSEL = 0x0
...
/*
 * The normal SPL console is brought up only after ps7_init() returns and the
 * SPL stack has moved to DDR.  Keep this tiny UART1 writer independent of DDR
 * so early PS bring-up failures can still be identified on the board header.
 * UART1 is connected to EBAZ4205 J7 through MIO 24/25; the divisor values are
 * the ones generated in this XSA for a 100 MHz UART reference clock.
 */
#define EBAZ4205_UART1_BASE       0xE0001000U
#define EBAZ4205_UART_CR          (EBAZ4205_UART1_BASE + 0x00U)
#define EBAZ4205_UART_MR          (EBAZ4205_UART1_BASE + 0x04U)
#define EBAZ4205_UART_BAUDGEN     (EBAZ4205_UART1_BASE + 0x18U)
#define EBAZ4205_UART_STATUS      (EBAZ4205_UART1_BASE + 0x2CU)
#define EBAZ4205_UART_FIFO        (EBAZ4205_UART1_BASE + 0x30U)
#define EBAZ4205_UART_BAUDDIV     (EBAZ4205_UART1_BASE + 0x34U)
#define EBAZ4205_UART_TXFULL      0x10U

static void ebaz4205_early_uart_init(void)
{
  mask_write(EBAZ4205_UART_BAUDDIV, 0x000000FFU, 0x00000006U);
  mask_write(EBAZ4205_UART_BAUDGEN, 0x0000FFFFU, 0x0000007CU);
  mask_write(EBAZ4205_UART_CR, 0x000001FFU, 0x00000017U);
  mask_write(EBAZ4205_UART_MR, 0x000003FFU, 0x00000020U);
}

static void ebaz4205_early_uart_putc(char value)
{
  volatile unsigned long *status =
      (volatile unsigned long *)EBAZ4205_UART_STATUS;
  volatile unsigned long *fifo =
      (volatile unsigned long *)EBAZ4205_UART_FIFO;

  while (*status & EBAZ4205_UART_TXFULL) {
  }
  *fifo = (unsigned long)value;
}

void ebaz4205_early_uart_puts(const char *text)
{
  volatile unsigned long *status =
      (volatile unsigned long *)EBAZ4205_UART_STATUS;

  while (*text != '\0') {
    ebaz4205_early_uart_putc(*text++);
  }

  /* Do not let the following UART reset discard the end of a marker. */
  while (((*status & 0x00000008U) == 0U) ||
         ((*status & 0x00000800U) != 0U)) {
  }
}
...
ps7_init(void) 
{
  ...
  ebaz4205_early_uart_init();
  ebaz4205_early_uart_puts("\r\nEBAZ4205 SPL: clock+UART OK\r\n");

  // DDR init
  ret = ps7_config (ps7_ddr_init_data);
  if (ret != PS7_INIT_SUCCESS) {
    ebaz4205_early_uart_puts("EBAZ4205 SPL: DDR init FAILED\r\n");
    return ret;
  }
  ebaz4205_early_uart_puts("EBAZ4205 SPL: DDR init OK\r\n");
  ...
  if (ret != PS7_INIT_SUCCESS) return ret;
  ebaz4205_early_uart_puts("EBAZ4205 SPL: PS7 init OK\r\n");
